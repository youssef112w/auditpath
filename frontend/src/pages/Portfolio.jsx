...
export default function Portfolio({ notify }) {
  ...
  useEffect(() => {
    Promise.all([api.get('/stats'), api.get('/audits'), api.get('/challenges')])
      .then(([s, a, c]) => {
    ...
    <div>
      ...
      <div className="mentor-card">
        ...
        <div className="mentor-tip">
          الـ Portfolio هو <strong>إثبات الإنتاج</strong>. مش CV. كل audit حقيقي وكل writeup تنشره هو استثمار.{' '}
          <strong>لا تنتظر ما تكون "مستعد".</strong> ابدأ النشر من أول شهر.
        </div>
      </div>

      <div style={{
        background: 'linear-gradient(135deg,rgba(0,255,136,0.08),rgba(124,58,237,0.06))',
        border: '1px solid rgba(0,255,136,0.2)',
        borderRadius: 16, padding: 28, marginBottom: 24
      }}>
        <div style={{ fontFamily: 'var(--font-code)', fontSize: 22, fontWeight: 700, color: 'var(--accent)', marginBottom: 4 }}>
          Smart Contract Security Researcher
        </div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--text3)', letterSpacing: 2, marginBottom: 4 }}>
          @{user?.username}
        </div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--text3)', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 20 }}>
        ...
        <div className="card">
          ...
          {audits.length === 0 ? (
            <div className="empty-state" style={{ padding: '24px 0' }}>
              ...
              <div style={{ fontSize: 11 }}>لا يوجد بعد</div>
            </div>
      ...
      <div className="card" style={{ marginTop: 16 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 8 }}>
          <div style={{ fontFamily: 'var(--font-code)', fontSize: 13, color: 'var(--accent)', flex: 1, wordBreak: 'break-all' }}>
            {publicURL}
          </div>
          <button className="btn btn-secondary" onClick={copyURL} style={{ whiteSpace: 'nowrap', minWidth: 80 }}>
            {copied ? '✓ تم' : '📋 نسخ'}
          </button>
        </div>
