...
export default function Challenges({ notify }) {
  ...
  const [filterSolved, setFilterSolved]     = useState('ALL') // ALL / solved / pending
  ...
  const filtered = challenges.filter(c => {
    ...
    const matchS = filterSolved === 'ALL' || (filterSolved === 'solved' ? c.solved : !c.solved)
  ...
  return (
    <div>
      ...
      <div className="mentor-card">
        ...
        <div className="mentor-tip"><strong>Ethernaut</strong> هو بداية مثالية. ابدأ بيه. كل level فيه درس اكتب عنه writeup حتى لو مختصر.</div>
      </div>
      ...
      <div style={{display:'flex',gap:8,flexWrap:'wrap',marginBottom:12}}>
        {['ALL',...PLATFORMS].map(p => (
          <button key={p} onClick={() => setFilterPlatform(p)} style={{
            ...
            background: filterPlatform===p ? 'var(--accent2)' : 'transparent',
            color: filterPlatform===p ? '#000' : 'var(--text3)',
            borderColor: filterPlatform===p ? 'var(--accent2)' : 'rgba(255,255,255,0.1)',
          }}>{p}</button>
        ))}
      </div>
      <div style={{display:'flex',gap:8,marginBottom:16}}>
        {[['ALL','الكل'],['solved','محلول ✓'],['pending','غير محلول']].map(([val,label]) => (
          <button key={val} onClick={() => setFilterSolved(val)} style={{
            fontFamily:'var(--font-mono)',fontSize:10,letterSpacing:1,padding:'4px 14px',borderRadius:20,cursor:'pointer',border:'1px solid',
            background: filterSolved===val ? 'var(--accent)' : 'transparent',
            color: filterSolved===val ? '#000' : 'var(--text3)',
            borderColor: filterSolved===val ? 'var(--accent)' : 'rgba(255,255,255,0.1)',
          }}>{label}</button>
        ))}
      ...
      {filtered.length === 0 ? (
        <div className="empty-state"><div className="empty-icon">🏆</div>{challenges.length === 0 ? 'ابدأ بـ Ethernaut Level 1!' : 'مفيش نتايج'}</div>
      ) : (
        filtered.map(c => (
