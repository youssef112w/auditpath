...
const EMPTY = { name:'', link:'', problem:'', cause:'', fix:'', severity:'medium', type:'ctf', notes:'', isPublic:false }
...
export default function AuditLab({ notify }) {
  ...
  const openEdit = (e, a) => {
    e.stopPropagation()
    setForm({ name:a.name, link:a.link||'', problem:a.problem, cause:a.cause||'', fix:a.fix||'', severity:a.severity, type:a.type, notes:a.notes||'', isPublic:a.isPublic, _id:a.id })
    setEditMode(true)
    setShowModal(true)
    setDetail(null)
  }
  ...
  const save = async () => {
    ...
    try {
      ...
      if (editMode) {
        const res = await api.put(`/audits/${form._id}`, { ...form, date })
        setAudits(prev => prev.map(a => a.id === form._id ? res.data : a))
        notify('تم التعديل ✓')
      } else {
  ...
  return (
    <div>
      ...
      <div className="flex-between mb-16">
        ...
        <button className="btn btn-primary" onClick={openAdd}>+ سجل عقد جديد</button>
      </div>
      {audits.length === 0 ? (
          ...
          <div key={a.id} className="entry-card" onClick={() => setDetail(a)}>
            <div className="flex-between">
              ...
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                ...
                <button className="btn btn-sm btn-secondary" onClick={e => openEdit(e, a)} style={{padding:'2px 10px',fontSize:11}}>تعديل</button>
              </div>
            </div>
      ...
      {showModal && (
        <Modal title={editMode ? `تعديل: ${form.name}` : '+ تسجيل عقد جديد'} onClose={() => setShowModal(false)}>
